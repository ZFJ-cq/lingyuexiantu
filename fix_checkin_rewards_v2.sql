...
-- ============================================
-- 3. 为角色初始化资产记录 (使用 role_assets 表)
-- ============================================

-- 获取所有角色 ID
SELECT '=== 现有角色列表 ===' AS info;
SELECT id, name, user_id FROM game_role WHERE is_deleted = 0 OR is_deleted IS NULL;

-- 为每个角色初始化灵石资产 (如果没有的话)
INSERT INTO role_assets (role_id, asset_type_id, quantity, created_at, updated_at)
SELECT 
    gr.id AS role_id,
...
WHERE at.code = 'LINGSHI'
  AND gr.is_deleted = 0 OR gr.is_deleted IS NULL
  AND NOT EXISTS (
...
SET quantity = quantity + 100,
    updated_at = NOW()
WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'LINGSHI');

-- 手动为角色 45 增加修为 (模拟签到奖励)
UPDATE role_assets 
SET quantity = quantity + 500,
    updated_at = NOW()
WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'XIUWEI');
...
SELECT 
    '灵石' AS asset_name,
    (SELECT quantity FROM role_assets WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'LINGSHI')) AS quantity
UNION ALL
SELECT 
    '修为' AS asset_name,
    (SELECT quantity FROM role_assets WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'XIUWEI')) AS quantity
UNION ALL
SELECT 
    '寿命' AS asset_name,
    (SELECT quantity FROM role_assets WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'SHOUMING')) AS quantity
UNION ALL
SELECT 
    '魂石' AS asset_name,
    (SELECT quantity FROM role_assets WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'HUNSHI')) AS quantity
UNION ALL
SELECT 
    '仙石' AS asset_name,
    (SELECT quantity FROM role_assets WHERE role_id = 45 AND asset_type_id = (SELECT id FROM asset_types WHERE code = 'XIANSHI')) AS quantity;
