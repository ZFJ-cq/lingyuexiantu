...
CREATE TABLE IF NOT EXISTS cfg_realm_attribute_mult (
    ...
    realm_level INT NOT NULL COMMENT '境界等级 (0=凡人，1=炼气，2=筑基，3=金丹，4=元婴，5=化神，6=炼虚，7=合体，8=大乘，9=渡劫)',
...
CREATE TABLE IF NOT EXISTS t_role_attribute_cache (
    ...
    hp BIGINT DEFAULT 0 COMMENT '当前生命值',
    hp_max BIGINT DEFAULT 0 COMMENT '最大生命值',
    mp BIGINT DEFAULT 0 COMMENT '当前法力值',
    ...
    equipment_bonus JSON COMMENT '装备加成 JSON',
    skill_bonus JSON COMMENT '技能加成 JSON',
    buff_bonus JSON COMMENT 'Buff 加成 JSON',
    calc_version BIGINT DEFAULT 0 COMMENT '计算版本号（用于缓存失效）',
    calculated_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '计算时间',
    expires_at DATETIME COMMENT '过期时间',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_calculated_at (calculated_at),
    INDEX idx_calc_version (calc_version)
...
-- 假设角色 45 的属性：根骨 120, 灵力 100, 身法 100, 悟性 15, 气运 50, 境界：炼气 (1)
...
SELECT 
    45,
    -- HP = (120 × 100) × 1.5 = 18000
    18000, 18000,
    -- MP = 灵力 × 50 = 5000
    5000, 5000,
    -- ATK = (100 × 8 + 120 × 1) × 1.2 = 1104
    1104,
    -- DEF = (120 × 5 + 100 × 2) × 1.2 = 960
    960,
    -- Speed = 100 × 10 = 1000
    1000,
    -- Crit = (50 × 0.001 + 100 × 0.0002) × 100 = 7%
    7.00,
    -- Dodge = (100 × 0.005) × 100 = 5%
    5.00,
    -- Exp = 1.0 + (15 × 0.01) = 1.15
    1.15,
    120, 100, 100, 15, 50,
    1,
    NOW()
...
-- 完成消息
-- ============================================
SELECT '✅ 属性计算和寿命系统创建完成！' AS message;
SELECT CONCAT('📊 属性规则数量：', COUNT(*)) AS msg FROM cfg_attribute_rules;
SELECT CONCAT('📊 境界倍率数量：', COUNT(*)) AS msg FROM cfg_realm_attribute_mult;
SELECT CONCAT('📊 属性缓存角色数：', COUNT(*)) AS msg FROM t_role_attribute_cache;
